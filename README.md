...
- 会话置顶、归档、复制为分支、跨分区移动
- 根据会话运行状态显示不同颜色提示：运行中为绿色、等待交互为红色、未查看的已完成会话为橙色；查看过的完成会话显示为灰色
- 折叠侧边栏时显示工作 / 生活快捷按钮
- 分区、置顶和归档数据保存在浏览器 `localStorage` 的 `dsh.wzone.v1`；已完成状态保存在 `dsh.wzone.status.v1`，刷新和重启 DSH 不会丢失
...
重启后，侧边栏中会出现「工作 / 生活」切换按钮。项目会按 DSH 工作区分组，会话数据由 DSH 提供，分区、置顶、归档偏好和完成提醒保存在浏览器本地。运行中为绿色、等待交互为红色、未查看的已完成会话为橙色；查看过的完成会话显示为灰色。
...
- **数据不见了**：插件数据位于当前浏览器 origin 的 `localStorage` 键 `dsh.wzone.v1` 和 `dsh.wzone.status.v1`；不要清除站点数据。
